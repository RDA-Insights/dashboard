...
# PAGE CONFIG
# -----------------------------------------------------------------------------
st.set_page_config(page_title="RDA Insights - Player Data Dashboard", layout="wide")
...
@st.cache_resource
def load_wta_image():
    url = "https://github.com/RDA-Insights/dashboard/raw/main/wtatransd.png"
...
def create_pass_and_carry_sonar(
    ...
    with close_after(fig):
        ...
        add_image(wtaimaged, fig, left=0.228, bottom=0.4535, width=0.05, alpha=0.25)
        add_image(wtaimaged, fig, left=0.7225, bottom=0.4535, width=0.05, alpha=0.25)
...
def create_player_actions_figure(
    ...
    with close_after(fig):
        ...
        add_image(wtaimaged, fig, left=0.415, bottom=-0.05075, width=0.04, alpha=1)
        plt.close(fig)
        return fig
...
def main():
    ...
    wtaimaged = Image.open(
        requests.get(
            "https://github.com/RDA-Insights/dashboard/raw/main/wtatransd.png",
            stream=True,
        ).raw
    )
    # -------------------------------------------------------
